...
-   Register a new application using the [Microsoft App Registration
    ...
    <details>
      <summary>Click to view vital information about registering the application</summary>
      <p> 
...
1.  Download the [Office365 connector from the WSO2 Connectors
    Store](https://store.wso2.com/store/assets/isconnector/details/5b67b248-fa00-4178-bf16-5606876ace4b).
...
5.  Configure the following fields.
    ...
    ![outbount-provisioning-connector-config](img/outbount-provisioning-connector-config.png)
    ...
    <p>:clipboard:<strong>Note:</strong> The claim URI for the Immutable ID should match the Subject Claim URI given under the Claim Configuration section when creating a service provider.</p>
    </div></td>
    ...
    <p>:clipboard:<strong>Note:</strong> This is an optional configuration and can be used when dynamically assigning users into groups<br />
...
6.  **Optional step** - you can provision users based on the roles they
    ...
    2.  Enter the provisioning roles.  
        ![role-configurations](img/role-configurations.png) 
...
3.  Click **Edit** on the **Display Name** claim and select **Supported By Default** to enable the claim.  
    ![claim-config](img/claim-config.png) 
...
1.  Create a group in the Azure AD. For more information, see [Create a
    ...
    >:information_source:When creating groups in the Azure AD, rules can be applied to  determine the membership based on user properties. 
    >  All the dynamic  group rules are evaluated in all additions/removals to the group. Dynamic group membership 
    >  reduces the administrative overhead of adding and removing users.

2.  Select **Dynamic Use** r as the **Membership type** when creating
    the group.

    >:information_source:You need to have a Azure AD Premium P1 license to add dynamic membership rules in Azure AD.
    
3.  Add a **Dynamic Membership Rule** as shown below. This rule
    specifies that any users that belong to the `Engineering` should be provisioned directly to the `Engineering` user group.  
    ![dynamic-membership-rules](img/dynamic-membership-rules.png) 
...
5.  Assign the user 'john' to the roles '
    `          office365role         ` ' and '
    `          Engineering         ` .  
    ![role-assigning](img/role-assigning.png) 
...
![provisioned-user](img/provisioned-user.png) 
